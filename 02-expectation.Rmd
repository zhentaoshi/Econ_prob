...
Jensen's inequality says that if $\varphi\left(\cdot\right)$ is a convex
    ...
    -   Application: Jensen's inequality is what works behind the maximum likelihood estimation.
    We almost always work will the log-likelihood. Maximizing the log-likelihood is
    equivalent to minimizing the negative log-likelihood. Notice that
    $-\log(\cdot)$ is a strictly convex function. Define *Kullback-Leibler distance*
    as         $d\left(p,q\right)=\int\log\left(p/q\right)\mathrm{d}P=E_{p}\left[\log\left(p/q\right)\right]$ for two probability measures $p$ and $q$. The Kullback-Leibler distance $d(p,q)\geq 0$ and the inequality holds if and only if  $p=q$.  

-   *Markov inequality* is another simple but important effect. If $E\left[\left|X\right|^{r}\right]$ exists,
    then
    ...
    for all $r\geq1$. *Chebyshev inequality*
        ...
        is a special case of the Markov inequality when $r=2$. Using the Chebyshev inequality is the
        simplest way to prove weak laws of large numbers.

# Multivariate Random Variable

Up to now we have been talking about univariate random variables.
We can extend the notion to a vector of random variables. A bivariate random variable is a
...
Joint PDF is defined similarly. It is often easier to work with the joint PDF
if it exists.

It is sufficient to introduce the joint distribution, conditional distribution
and marginal distribution in the simple bivariate random, and these definitions
can be easily extended to multivariate distributions. Suppose
a bivariate random variable $(X,Y)$ has a joint density
$f(\cdot,\cdot)$. We have these familiar formulae:

-   *conditional density* can be roughly written as  $f\left(y|x\right)=f\left(x,y\right)/f\left(x\right)$ if we do not formally deal
with the case $f(x)=0$.
...
which I choose not to do at this time. The first inequality can be interpreted
as if we standardize $P(A_1 A_2)$ by the total mass $P(A_2)$; $A_2$ plays
the role as the outcome space or the universe in the conditional probability.
...
**Example:** $A_1$ is the event "a student can survive CUHK's MSc program", and $A_2$ is the application profile.
...
If a conditional density exists, the *conditional expectation* can be computed as
    $E\left[Y|X\right]=\int yf\left(y|X\right)dy$.

*Law of iterated expectation*
$$E\left[E\left[Y|X\right]\right]=E\left[Y\right]$$
is a trivial fact from the definition of the conditional expectation by taking
$A=\Omega$.
...
**Application**: Regression is a technique that decompose a random variable $Y$
